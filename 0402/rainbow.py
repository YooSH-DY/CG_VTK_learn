...
def get_program_parameters():
    # 현재 디렉토리에 있는 파일 경로 지정
    import os

    current_dir = os.path.dirname(os.path.abspath(__file__))
    xyz_filename = os.path.join(current_dir, "combxyz.bin")
    q_filename = os.path.join(current_dir, "combq.bin")

    # 파일이 존재하는지 확인 (선택사항)
    if not os.path.exists(xyz_filename) or not os.path.exists(q_filename):
        print(f"Warning: Data files not found in {current_dir}")
        print("Expected files: combxyz.bin, combq.bin")

    return xyz_filename, q_filename
